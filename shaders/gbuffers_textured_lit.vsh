...
void main() {
	...
	float lightDot = dot(normalize(shadowLightPosition), normalize(gl_NormalMatrix * gl_Normal));
		...
		//when EXCLUDE_FOLIAGE is enabled, act as if foliage is always facing up.
		//in other words, don't darken the back side of it unless something else is casting a shadow on it.
		if (mc_Entity.x == 10000.0) lightDot = dot(normalize(shadowLightPosition), normalize(upPosition));;
	#endif
